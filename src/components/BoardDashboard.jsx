          ...
          <div className="bg-white/60 backdrop-blur-2xl rounded-3xl shadow-[0_30px_60px_-15px_rgba(0,0,0,0.3)] p-8 w-full max-w-[420px] relative flex flex-col animate-scale-up">
            ...
            <form onSubmit={handleCreateSubmit} className="relative z-10 space-y-4">
              {showModal.type === 'post_add' ? (
                <>
                  <input type="text" autoFocus value={inputText} onChange={(e) => setInputText(e.target.value)} placeholder="제목을 입력하세요..." className="w-full bg-white/50 border border-white/60 text-gray-800 text-sm font-medium rounded-2xl px-4 py-3.5 outline-none focus:border-blue-400 shadow-inner" />
                  <div className="relative z-20">
                    <label className="text-xs font-bold text-gray-500 mb-1.5 block">카테고리 선택</label>
                    <div className="flex items-center space-x-2 mb-2">
                      <div className="relative flex-1">
                        <button type="button" onClick={() => setIsDropdownOpen(!isDropdownOpen)} className="w-full flex items-center justify-between bg-white/50 border border-white/60 text-gray-800 text-sm font-medium rounded-2xl px-4 py-3.5 outline-none hover:bg-white/80">
                          <span>{mediumCats.find(c => c.id === selectedMediumId)?.name || '폴더 선택'}</span>
                          <ChevronDown className={`w-4 h-4 transition-transform ${isDropdownOpen ? 'rotate-180' : ''}`} />
                        </button>
                        <div className={`absolute left-0 right-0 top-full mt-2 bg-white/80 backdrop-blur-xl border border-white/60 rounded-2xl shadow-xl z-50 transition-all ${isDropdownOpen ? 'opacity-100 visible' : 'opacity-0 invisible'}`}>
                          {mediumCats.map(cat => (
                            <div key={cat.id} onClick={() => { setSelectedMediumId(cat.id); setIsDropdownOpen(false); }} className="px-4 py-2.5 text-sm cursor-pointer hover:bg-blue-50">
                              {cat.name}
                            </div>
                          ))}
                        </div>
                ...
                <input type="text" autoFocus value={inputText} onChange={(e) => setInputText(e.target.value)} maxLength={15} placeholder="이름을 입력하세요..." className="w-full bg-white/50 border border-white/60 text-gray-800 text-sm font-medium rounded-2xl px-4 py-4 outline-none focus:border-blue-400" />
              )}
              <div className="flex space-x-3 mt-8">
                <button type="button" onClick={() => setShowModal({ type: null, targetId: null })} className="flex-1 bg-white/70 text-gray-600 text-sm font-bold py-3.5 rounded-2xl hover:bg-white border border-white/60">취소</button>
                <button type="submit" className="flex-1 bg-gray-900 text-white text-sm font-bold py-3.5 rounded-2xl">확인</button>
              </div>
            </form>
          </div>
        </div>
            ...
            <div className="bg-white/60 backdrop-blur-2xl rounded-3xl shadow-[0_30px_60px_-15px_rgba(0,0,0,0.3)] p-8 w-full max-w-[420px] relative flex flex-col animate-scale-up">
              ...
              <form id="boardCreateForm" onSubmit={handleCreateSubmit} className="relative z-10 space-y-4">
                {showModal.type === 'post_add' ? (
                  ...
                  <input type="text" autoFocus value={inputText} onChange={(e) => setInputText(e.target.value)} maxLength={15} placeholder="이름을 입력하세요..." className="w-full bg-white/50 border border-white/60 text-gray-800 text-sm font-medium rounded-2xl px-4 py-4 outline-none focus:bg-white/80 focus:border-blue-400 focus:ring-4 focus:ring-blue-400/20 caret-blue-600 transition-all duration-300 shadow-inner placeholder:text-gray-400" />
                )}
                <div className="flex space-x-3 mt-8">
                  <button type="button" onClick={() => { setInputText(''); setNewCategoryName(''); setIsDropdownOpen(false); setShowModal({ type: null, targetId: null }); }} className="flex-1 bg-white/70 backdrop-blur-sm text-gray-600 text-sm font-bold py-3.5 rounded-2xl hover:bg-white hover:text-gray-800 transition-colors border border-white/60 shadow-sm">취소</button>
                  <button 
                    type="submit" 
